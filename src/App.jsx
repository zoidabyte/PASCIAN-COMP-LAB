...
export default function App() {
  ...
  return (
    <div className={`min-h-screen w-full relative transition-colors duration-500 overflow-hidden font-sans antialiased ${theme.base}`}>
      ...
      {/* Screwdriver Modal (Preserved & Themed) */}
      {isSDModalOpen && (
      ...
      <div className="relative z-10 flex h-screen w-full backdrop-blur-[2px]">
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8">
          <div className="max-w-7xl mx-auto h-full flex flex-col space-y-6">
            ...
            {/* Central Semi-Transparent View Rendering */}
            ...
            {/* VIEW 3: ADMIN PORTAL (BORROWED & HISTORY) */}
              ...
              <div className="animate-in fade-in duration-300">
                {!isAdminUnlocked ? (
                  <div className={`${theme.card} max-w-sm mx-auto mt-20 p-8 rounded-3xl shadow-xl border text-center backdrop-blur-xl`}>
                    ...
                    <h2 className={`text-xl font-black mb-2 ${theme.textMain}`}>Admin Access</h2>
                    <p className={`text-sm mb-6 ${theme.textMuted}`}>Enter your security PIN to access the dashboard.</p>
                    <form onSubmit={handleAdminLogin} className="space-y-4">
                      <input 
                        ...
                        className={`w-full text-center tracking-widest text-2xl font-mono rounded-xl px-4 py-4 ${theme.input}`}
                        autoFocus
                      />
                      <button type="submit" className="w-full bg-indigo-600 text-white py-4 rounded-xl font-black shadow-md shadow-indigo-500/30 hover:bg-indigo-500 active:scale-95 transition-all">Unlock</button>
                    </form>
                  </div>
                ) : (
